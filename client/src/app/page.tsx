...
const Home = () => {
  ...
  const joinLobby = (e: FormEvent) => {
    ...
    window.localStorage.setItem("sprite", characterUrls[urlIndex].split("/")[2]);
    router.push("/game");
  };

  return (
    <div className="bg-base min-h-[100vh] grid place-items-center">
      <div className="p-4 px-8 bg-highlight rounded-2xl flex gap-x-16 items-center justify-center overflow-hidden 2xl:w-1/2">
        <div>
          <p className="text-lg lg:text-2xl font-semibold text-center">Choose your character</p>
          <div className="flex items-center relative mt-3 rounded-lg">
            <Image
            ...
            <div className="flex w-full justify-between absolute">
              <FaCircleChevronLeft
                ...
                onClick={() => setUrlIndex((prev) => (prev == 0 ? 2 : prev - 1))}
              />
              <FaCircleChevronRight
                size={30}
                className="cursor-pointer hover:scale-110 transition-all hover:fill-text-base"
                onClick={() => setUrlIndex((prev) => (prev == 2 ? 0 : prev + 1))}
              />
            </div>
            ...
            <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex">
              {characterUrls.map((url, index) => (
                <GoDotFill
                  className={`cursor-pointer ${index == urlIndex ? "fill-text-base" : ""}`}
                  size={20}
                  key={index}
                />
              ))}
        ...
        <div>
          ...
          <form onSubmit={joinLobby}>
            ...
            />
            <Button_default text={"Join"} className={"px-8 font-bold mt-2 ml-4 block"} onClick={joinLobby} />
          </form>
        </div>
      </div>
    </div>
